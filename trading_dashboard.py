"""
Deepcharts-Style Order Flow Terminal
Footprint · Delta Profile · Heat Profile · Volume Profile · CVD · Absorption
...
.dc-meta { color: #6b7280; font-size: 0.72rem; font-family: 'JetBrains Mono', monospace; }
...
/* Footprint table */
.fp-table { width: 100%; border-collapse: collapse; font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; }
.fp-table th { color: #f5c542; text-align: left; padding: 4px 6px; border-bottom: 1px solid #1e2330; }
.fp-table td { padding: 3px 6px; border-bottom: 1px solid #161a24; color: #c5c9d0; }
.fp-buy { color: #22c55e; }
...
def fetch_ohlc(symbol, period="5d", interval="15m"):
    try:
        ...
        df = df.reset_index()
        c0 = df.columns[0]
        df = df.rename(columns={c0: "time", "Open": "open", "High": "high",
...
def enrich_orderflow(df):
    ...
    # Buy vol ≈ volume * (close-low)/range ; Sell vol ≈ volume * (high-close)/range
    # Classic approximation used when tick data unavailable
...
def build_volume_profile(df, bins=56, va_pct=0.70):
...
def detect_absorption(df, min_vol_mult=1.8, max_body_pct=0.35):
    ...
    for i in range(len(d)):
        ...
        if av <= 0 or row.get("range", 0) is None or (isinstance(row.get("range"), float) and (pd.isna(row["range"]) or row["range"] <= 0)):
            continue
...
def heat_color(intensity, positive=True):
    """Map 0-1 intensity to heat colors (green buy / red sell)."""
    intensity = float(np.clip(intensity, 0, 1))
    if positive:
        # dark green -> bright green
        r = int(10 + (34 - 10) * intensity)
        g = int(40 + (197 - 40) * intensity)
        b = int(40 + (94 - 40) * intensity)
    else:
        r = int(40 + (239 - 40) * intensity)
        g = int(20 + (68 - 20) * intensity)
        b = int(20 + (68 - 20) * intensity)
    return f"rgb({r},{g},{b})"

# ====================== SIDEBAR ======================
with st.sidebar:
    st.markdown("### DEEP ORDER FLOW")
    st.caption("Deepcharts-style controls")
    st.markdown("---")
    ...
    rate = st.slider("Seconds", 20, 120, 40)
    ...
    st.markdown("---")
    st.markdown("### PROFILE")
    lookback = st.slider("Lookback bars", 50, 250, 120, 10)
    n_bins = st.slider("Profile bins", 32, 96, 56, 4)
    ...
    chart_h = st.slider("Chart height px", 520, 900, 720, 20)
...
# ====================== MAIN LAYOUT: BIG CHART + SIDE PANEL ======================
main_col, side_col = st.columns([2.4, 1])

with main_col:
    st.markdown('<div class="dc-panel-title">ORDER FLOW CHART — FOOTPRINT / PROFILE / HEAT</div>', unsafe_allow_html=True)

    if ohlc is None or len(ohlc) < 5:
        st.warning("No data. Click REFRESH or change instrument.")
    else:
        # Subplots: price+profile | CVD
        rows = 2 if show_cvd else 1
        row_heights = [0.72, 0.28] if show_cvd else [1.0]
        fig = make_subplots(
            rows=rows, cols=2,
        ...
        # Footprint delta markers on candles (size ~ |delta|)
        if show_fp_overlay:
            max_abs_d = ohlc["delta"].abs().max() + 1e-9
            sizes = 6 + 14 * (ohlc["delta"].abs() / max_abs_d)
            colors = ["#22c55e" if d >= 0 else "#ef4444" for d in ohlc["delta"]]
            fig.add_trace(go.Scatter(
                x=ohlc["time"], y=ohlc["close"],
                mode="markers",
                marker=dict(size=sizes, color=colors, opacity=0.35, symbol="circle"),
                name="Bar Δ",
        ...
        if show_vwap and vwap_val:
            fig.add_hline(y=vwap_val, line_dash="dot", line_color="#f5c542", line_width=1.4,
                          annotation_text=f"VWAP {vwap_val:.{dec}f}", annotation_position="top right",
                          row=1, col=1)
        ...
        if profile:
            ...
            if show_va:
                ...
                fig.add_hline(y=profile["vah"], line_dash="dash", line_color="#22c55e", line_width=1.2,
                              annotation_text="VAH", annotation_position="top left", row=1, col=1)
                fig.add_hline(y=profile["val"], line_dash="dash", line_color="#ef4444", line_width=1.2,
                              annotation_text="VAL", annotation_position="bottom left", row=1, col=1)

            if show_hvn:
                for h in hvn_list[:5]:
                    fig.add_hline(y=h["price"], line_color="rgba(56,189,248,0.65)",
                                  line_width=1, line_dash="dot", row=1, col=1)
            if show_lvn:
                for lv in lvn_list[:4]:
                    fig.add_hline(y=lv["price"], line_color="rgba(167,139,250,0.5)",
                                  line_width=1, line_dash="dot", row=1, col=1)

            # --- Volume / Delta / Heat PROFILE (right) ---
            if show_vp or show_delta_profile:
                max_v = profile["volume"].max() + 1e-9
                max_ad = np.abs(profile["delta"]).max() + 1e-9
        ...
        if show_cvd:
            ...
            # zero line
            fig.add_hline(y=0, line_color="#2a3142", line_width=1, row=2, col=1)
        ...
        fig.update_yaxes(showgrid=True, gridcolor="#1a1f2e", color="#8b919a", row=1, col=1)
        fig.update_yaxes(showgrid=False, color="#8b919a", row=1, col=2)
        if show_cvd:
            fig.update_yaxes(showgrid=True, gridcolor="#1a1f2e", title_text="CVD", row=2, col=1)
...
| **VAH / VAL** | Value Area high/low (acceptance zone) |
...
| **Footprint table** | Per-bar buy/sell/delta approximation |
